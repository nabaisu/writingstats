...
filename = time.strftime("%Y-%m-%d")+"-reflexoes-" + str(i) + "o-pomodoro-"+ ".md"
...
if not adeus:
    ...
    print '[*] - Opening File ...'
    subprocess.call(bash_command1,shell=True)
    if path.exists("/Applications/SelfControl.app") and path.exists(os.path.dirname(os.path.abspath(__file__))+"/self-c.applescript"):
        bash_command2 = "osascript "+os.path.dirname(os.path.abspath(__file__))+"/self-c.applescript 30"
        print '[*] - Closing Down the Internet... See you in 30 minutes'
        subprocess.call(bash_command2,shell=True)
